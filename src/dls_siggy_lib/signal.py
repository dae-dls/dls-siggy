...
class Signal:
    ...
    def _signal_handler(self, sig, frame):
        self._count += 1
        logger.debug("signum %d happened, count is now %d" % (self._signum, self._count))
        if self._auto_deactivate_count is not None and self._count >= self._auto_deactivate_count:
            logger.debug("signum %d auto deactivate count %d reached" % (self._signum, self._auto_deactivate_count))
            self.deactivate()
    ...
    def activate(self, auto_deactivate_count=None):
        ...
        logger.debug("signum %d activated to auto deactivate at count %s" % (self._signum, self._auto_deactivate_count))
